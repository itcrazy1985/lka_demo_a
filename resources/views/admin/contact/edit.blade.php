            ...
            <div class="card-body">
              <form role="form" class="text-start" action="{{ route('admin.contact.update', $contact->id) }}" method="post">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control border border-1 border-secondary px-3" id="" name="name" value="{{ $contact->name }}">
                    @error('name')
                    <span class="text-danger d-block">*{{ $message }}</span>
                    @enderror
                </div>
                <div class="mb-3">
                    <label class="form-label text-dark" for="text">Value</label>
                    <input type="text" class="form-control border border-1 border-secondary px-3" id="" name="value" value="{{ $contact->value }}">
                    @error('value')
                    <span class="text-danger d-block">*{{ $message }}</span>
                    @enderror
                </div>
